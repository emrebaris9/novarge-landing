...
function openPopup() {
  Swal.fire({
    title: "Eğitimle ilgili Yorumunu Paylaş",
    html: `
      <div class="form-group mb-3">
        <label for="swal-name" class="form-label">İsim Soyisim</label>
        <input type="text" id="swal-name" class="swal2-input" placeholder="İsim Soyisim giriniz">
      </div>
      <div class="form-group mb-3">
        <label for="swal-comment" class="form-label">Yorumunuz</label>
        <textarea id="swal-comment" class="swal2-textarea" placeholder="Eğitim hakkındaki yorumlarınızı girin."></textarea>
      </div>
      <div class="form-group mb-3">
        <label class="form-label">Değerlendirme Puanınızı 5 üzerinden giriniz</label>
    ...
    confirmButtonText: "Yorumu Gönder",
    confirmButtonColor: "#6A1B9A",
    confirmButtonClass: "w-100 rounded-pill", 
    ...
    grow: "row",
    didOpen: () => {
      // Rating butonları için click event listener'ları
    ...
    preConfirm: () => {
      ...
      const rating =
        document.querySelector(".rating-btn.active")?.dataset.value;

      if (!name) {
        Swal.showValidationMessage("Lütfen isim soyisim giriniz");
        return false;
      }
      if (!comment) {
      ...
      if (!rating) {
        Swal.showValidationMessage("Lütfen bir değerlendirme puanı seçin");
        return false;
      }

      // mailto link oluştur
      const mailtoLink = `mailto:info@novarge.com?subject=Yeni Yorum&body=${encodeURIComponent(
